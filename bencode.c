...
long long be_str_len(be_node *node) {
  long long ret = 0;
  if (node->val.s) memcpy(&ret, node->val.s - sizeof(ret), sizeof(ret));
  return ret;
}
...
void be_free(be_node *node) {
  switch (node->type) {
    ...
    case BE_LIST: {
      unsigned int i;
      for (i = 0; node->val.l[i]; ++i) be_free(node->val.l[i]);
      free(node->val.l);
      break;
    }
...
void _be_dump(be_node *node, ssize_t indent) {
  ...
  switch (node->type) {
    ...
    case BE_LIST:
      ...
      for (i = 0; node->val.l[i]; ++i) _be_dump(node->val.l[i], indent + 1);
